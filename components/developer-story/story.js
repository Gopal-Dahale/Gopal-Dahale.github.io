...
import { Text, VStack, Heading, Box, Image, HStack, Divider, IconButton } from '@chakra-ui/react';
      ...
      <VStack textAlign="start" align="flex-start" mb={0}>
        <Box>
          ...
          {companies.map((company, index) => (
            <StoryTimeline icon={BsFillBriefcaseFill} index={index} key={index}>
              ...
              <HStack>
                ...
                />
                <VStack align="start">
                  <Heading fontSize={[12, 13, 15]} lineHeight="shorter" fontWeight="bold">
          ...
          {institutes.map((institute, index) => (
            <>
              <StoryTimeline icon={FaGraduationCap} index={index > 0 ? index + 1 : index}>
                {' '}
                <HStack>
                  <Image
                    rounded="full"
